        ...
        <thead>
          <tr className="text-left text-white font-bold bg-[#005646]">
            ...
            <th className="p-1">
              <button
                onClick={handleNext}
                disabled={visibleIndex === data.length - 3}
            ...
            {data.slice(visibleIndex, visibleIndex + 3).map((day, index) => (
              <td key={index} className="px-4 py-2">
                {day.slots.map((slot, idx) => (
                  <div
                    key={idx}
                    className="my-3 p-2 bg-white rounded-lg border-[2px] border-white hover:font-bold hover:text-sm  hover:border-[#1C8F97] hover:border-[2px] "
                  >
                    {slot}
                  </div>
                ))}
